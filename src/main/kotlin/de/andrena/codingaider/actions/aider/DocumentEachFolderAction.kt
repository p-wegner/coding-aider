    ...
    companion object {
        fun documentEachFolder(project: Project, virtualFiles: Array<VirtualFile>) {
            ...
            virtualFiles.filter { it.isDirectory }.forEach { folder ->
                ...
                val allFiles = folder.children.filter { !it.isDirectory }
                val fileNames = allFiles.map { File(it.path).name }

                val commandData = CommandData(
                    message = """Generate a markdown documentation for the code in the folder $folderName: $fileNames. 
                        |If there are exceptional implementation details, mention them. Store the results in $folderName.md. 
                        |If the file already exists, update it instead.
                        |""".trimMargin(),
                    useYesFlag = true,
                    llm = settings.llm,
