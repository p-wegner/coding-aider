...
class DocumentationGenerationDialog(
    ...
    override fun createCenterPanel(): JComponent {
        val panel = panel {
            ...
            row {
                label("Enter filename for the documentation:")
            }
            row {
                cell(filenameField)
                    .resizableColumn()
    ...
    override fun doOKAction() {
        ...
        try {
            ...
            // Create or ensure the file exists
            val targetFile = if (selectedFiles.size == 1 && selectedFiles[0].isDirectory) {
                File(selectedFiles[0].path, filename)
            } else {
            ...
            val commandData = CommandData(
                message = buildPrompt(documentType, allFiles, targetFile.path),
                useYesFlag = true,
                files = allFiles + contextFiles + listOf(FileData(targetFile.path, false)),
    ...
    private fun buildPrompt(documentType: DocumentTypeConfiguration, files: List<FileData>, targetPath: String): String {
